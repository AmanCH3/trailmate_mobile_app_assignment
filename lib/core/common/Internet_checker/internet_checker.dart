...
class InternetChecker {
  ...
  static Future<bool> isConnected() async {
    ...
    if (_isConnectedType(result)) {
      try {
        print(
          '[InternetChecker] Attempting HTTP GET to google.com/generate_204',
        );
  ...
  static Stream<bool> get onStatusChange async* {
    await for (final result in _connectivity.onConnectivityChanged) {
      ...
      if (_isConnectedType(result)) {
        try {
          print(
            '[InternetChecker] onStatusChange: Attempting HTTP GET to google.com/generate_204',
          );
          final httpClient = HttpClient();
          httpClient.connectionTimeout = const Duration(seconds: 2);
          final request = await httpClient.getUrl(
          ...
          httpClient.close();
          print(
            '[InternetChecker] onStatusChange: HTTP status: ${response.statusCode}',
          );
        ...
        print(
          '[InternetChecker] onStatusChange: Not connected to WiFi or mobile',
        );
        yield false;
      }
    }
