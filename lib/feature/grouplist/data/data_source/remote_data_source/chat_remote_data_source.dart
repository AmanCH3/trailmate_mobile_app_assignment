...
class ChatRemoteDataSourceImpl implements IChatDataSource {
  ...
  void connectAndListen(String groupId) {
    ...
    _socket.onConnect((_) {
      ...
      // Join the specific group room once connected
      joinGroup(groupId);
    });
  ...
  Future<List<MessageApiModel>> getMessageHistory(String groupId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Throw a specific failure that the repository can catch
        throw ApiFailure(
      ...
      // Rethrow to be handled by the repository
      rethrow;
    }
  }
  ...
  Future<void> sendMessage(Map<String, dynamic> messageData) async {
    // Implemented: Emit message data via socket
    _socket.emit(ApiEndpoints.socketSendMessage, messageData);
  }

  @override
  void dispose() {
    // Implemented: Close stream and disconnect socket
